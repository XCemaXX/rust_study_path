...
mod colors {
        ...
        fn from_str(hex_color: &str) -> Result<Self, Self::Err> {
            ...
            let parse_hex = |s| {
                u8::from_str_radix(s, 16)
                    .map_err(|_| io::Error::new(ErrorKind::InvalidData, "bad hex"))
            };
            Ok(HexColor {
    ...
    pub fn transform_colors() -> Result<(), Box<dyn std::error::Error>> {
        println!("---Colors");
...
fn main() {
    read_csv::read().unwrap();
    filter_records::filter().unwrap();
    serialize::serialize().unwrap();
    colors::transform_colors().unwrap();
}
