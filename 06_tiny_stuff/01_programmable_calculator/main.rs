pub mod calculator {
    ...
    fn get_reg_value(
        ...
        Ok(match iter.next() {
            Some(s) => match s.as_ref().parse::<Value>() {
    ...
    fn parse_instruction(line: &str) -> Result<Instruction, String> {
        ...
        let res = match parts.next() {
            Some("MOVI") => {
                let (r1, v) = get_reg_value(&mut parts)?;
                Instruction::MOVI(r1, v)
            }
            Some("ADD") => {
                let (r1, r2) = get_two_regs(&mut parts)?;
