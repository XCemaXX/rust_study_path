...
impl MinHash {
    ...
    fn find_min(
        set: impl IntoIterator<Item = impl AsRef<str>>,
        hash_func: &Box<dyn Fn(&str) -> u32>,
    ) -> u32 {
    ...
    fn get_signature(
        &self,
        set: impl IntoIterator<Item = impl AsRef<str>> + std::marker::Copy,
    ) -> Vec<u32> {
    ...
    pub fn similarity<T: IntoIterator<Item = impl AsRef<str>> + std::marker::Copy>(
        &self,
        set_a: T,
        set_b: T,
    ) -> f64 {
