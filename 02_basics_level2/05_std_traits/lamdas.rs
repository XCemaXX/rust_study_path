...
fn run_with_log2<Args: Display, Res, Func: FnOnce(Args) -> Res>(
    call_back: Func,
    args: Args,
) -> Res {
...
fn main() {
    ...
    let increment = |x| {
        count += x;
        println!("Add to counter {x}; Counter: {count}");
    };
    ...
    let has_gc = {
        ...
        move |lang: &str| -> bool { !no_gc.contains(&lang) }
    };
    let lang = "Java";
